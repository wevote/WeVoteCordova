...
function writeCordovaLibGradleWrapperProperties () {
  ...
  try {
    if (!fs.existsSync(path)) {
      console.log('> Processed: Making directory ' + path0);
      fs.mkdirSync(path0);
      console.log('> Processed: Making directory ' + path);
      fs.mkdirSync(path);
    }
  } catch (err) {
...
const copyGoogleServices = () => {
  ...
  if (fs.existsSync(destinationFile)) {
    ...
    fs.copyFile(originalFile, destinationFile, fs.constants.COPYFILE_EXCL, (err) => {
      if (err) {
        console.log("> ERROR: Copy file error Found:", err);
      } else {
...
const copyJQueryFile = () => {
  ...
  if (fs.existsSync(iosFile)) {
    ...
    fs.copyFile(originalFile, iosFile, fs.constants.COPYFILE_EXCL, (err) => {
      if (err) {
        console.log("> ERROR: Copy file error for jQuery to iOS dir.  Found:", err);
      } else {
        console.log('> Created: Created file: ', iosFile);
      }
    });
  }
  if (fs.existsSync(androidFile)) {
    ...
    fs.copyFile(originalFile, androidFile, fs.constants.COPYFILE_EXCL, (err) => {
      if (err) {
        console.log("> Error: Copy file error for jQuery to Android dir.  Found:", err);
      } else {
        console.log('> Created: Created file: ', iosFile);
      }
    });
...
*************************************************************************************************/
{
  ...
  if ((myArgs.length === 0) || ((!myArgs[0].includes('/build')))) {
    console.log('> ERROR: Need to supply a path to the WebApp build directory!  For example:');
